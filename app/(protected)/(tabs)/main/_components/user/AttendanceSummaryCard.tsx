...
export default function AttendanceSummaryCard() {
  ...
  // 로그 수정
  console.log("🚨 쿼리 에러 상세:", {
    message: error?.message,
    response: (error as any)?.response?.data, // 서버가 보낸 에러 메시지
    status: (error as any)?.response?.status, // 401, 404, 500 등
  });
          ...
          <View className="w-full h-[180px] bg-[#F8FAFC] rounded-3xl p-6 mb-6">
            ...
            <View className="flex-row flex-1 justify-between items-end">
              {data.dailyGraph.map((item: any) => {
                const colors =
                  THEME_COLORS[item.status] || THEME_COLORS.DEFAULT;
                return (
